...
from src.model.transformer.Optim import ScheduledOptim
from src.training import ModifierLabelsDecoder
from src.evaluation.metrics import ExperimentMetrics
from src.evaluation.visdom_plotter import VisdomPlotter
...
class Train:
    ...
    def __init__(self, config: dict, engine: RenderEngine):
        ...
        self.train_metrics = ExperimentMetrics(modifier_class_labels=[m.name for m in ModifierEnum],
                                               ...
                                               data_type='Training')
        self.plotter = VisdomPlotter(experiment_env=config['EXPERIMENT_NAME'])

    def train(self):

        epochs = self.config['TRAIN']['EPOCHS']

        train_dataloader = create_dataloader(config=self.config, data_type='train', engine=self.engine)
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            try:
                ...
                if i % self.config['LOG_RATE'] == 0:
                    self.plotter.plot_images(rendered_triplet[0])
    ...
    def save_model(self, model):
        model_dir = os.path.join('output', self.config['EXPERIMENT_NAME'])
