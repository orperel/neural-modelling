...
from src.graphics.geometry.mesh_utils import vertex_distance, edges_angle
from src.graphics.geometry.mesh_iterator import iterate_faces_of_vertex_group, get_face_vertex_ids, get_vertex_neighbors
from src.graphics.modifiers.abstract_modifier import AbstractModifier


class SplitVertexTripletModifier(AbstractModifier):

    def __init__(self, mesh: Mesh, v_id, vl_id, vr_id, tx, ty, tz):
        super().__init__(mesh)
        self.v_id = v_id
        self.vl_id = vl_id
        self.vr_id = vr_id
    ...
    def _update_edges(self, v_id, v1, v2):
        ...
        v1_old = self.mesh.vertices[v1_id]
        angles = []
        ...
        for e_id in v_edges:
            ...
            angle = edges_angle(v1, v1_old, vi, v1_old)
            angles.append(angle)
