...
def edges_angle(va1, vb1, va2, vb2):
    edge_vector_1 = np.array(edge(vb1, va1))
    edge_vector_2 = np.array(edge(vb2, va2))
    cosine_ang = np.dot(edge_vector_1, edge_vector_2) / (np.linalg.norm(edge_vector_1) * np.linalg.norm(edge_vector_2))
    return np.arccos(cosine_ang)
